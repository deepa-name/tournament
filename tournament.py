...
def deleteMatches(tournamentId):
    """Remove all the match records for a tournament from the
    database."""
    conn, c = connect()
...
def deletePlayers(tournamentId):
    ...
    c.execute("DELETE FROM tournament_players WHERE tournament_id = "
              "(%s) RETURNING player_id", (tournamentId,))
    playerIds = c.fetchall()
    # For each of the entry removed earlier remove them from player table
    for p in playerIds:
        c.execute("DELETE FROM players WHERE player_id IN (%s)",
                  (p[0],))
    conn.commit()
    conn.close()
...
def registerPlayer(name, tournamentId):
    ...
    """
    conn, c = connect()
    # First insert the player into the player (check if user already
    # exists omitted because duplicate names are allowed)
    ...
    c.execute("INSERT INTO tournament_players (tournament_id, "
              "player_id) VALUES (%s,%s)", (tournamentId, playerId))
    conn.commit()
    conn.close()
...
def playerStandings(tournamentId):
    ...
    a player tied for first place if there is y a tie.
    ...
    c.execute("SELECT player_id, name, total_wins, total_matches "
              "FROM tournament_players_view WHERE tournament_id = "
              "(%s) ORDER BY total_wins DESC", (tournamentId,))
    players = c.fetchall()
    conn.close()
    return players
...
def reportMatch(winner, loser, tournamentId):
    ...
    c.execute("INSERT INTO matches (tournament_id, player1, "
              "player2, winner) VALUES (%s, %s, %s, %s)",
              (tournamentId, winner, loser, winner))
    # Then update the total_matches and total_wins of winner and loser
    c.execute("UPDATE tournament_players SET total_matches = "
              "(total_matches + 1) WHERE tournament_id = (%s) "
              "AND player_id = (%s)", (tournamentId, loser))
    c.execute("UPDATE tournament_players SET total_matches = "
              "(total_matches + 1), total_wins = (total_wins + 1) "
...
def swissPairings(tournamentId):
    ...
    c.execute("SELECT player_id, name FROM tournament_players_view  "
              "WHERE tournament_id = (%s) ORDER BY total_matches, "
    ...
    while i < len(players):
        if i % 2 != 0:
            pairings.append((prevPlayer[0], prevPlayer[1],
